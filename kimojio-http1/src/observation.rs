//! Optional per-connection logs and, with the `metrics` feature, live snapshots.
//!
//! Attach one handle through [`crate::Config::observation`] before polling the
//! driver. Logger callbacks run synchronously; snapshot requests require the
//! connection driver to remain polled.
