//! Bounded body streams and exclusive leases connecting applications to the driver.
//!
//! Outgoing sources are polled only when the protocol grants demand. Incoming
//! chunks retain receive storage until dropped or forwarded through a receipt.
use std::{
    cell::Cell,
    fmt,
    ops::Deref,
...
pub enum OutgoingFrame {
    /// Sends one payload frame; each allocation must fit the configured buffer limit.
    Data(Vec<u8>),
    ...
    Forward(BodyChunk),
    /// Ends a streaming body with validated HTTP trailer fields.
    Trailers(HeaderMap),
}
...
impl OutgoingBody {
    /// Creates a body with no payload; no producer demand is issued.
    pub fn empty() -> Self {
        Self {
    ...
    /// Creates a fixed-length body from one owned payload allocation.
    ///
    /// The payload must fit `Config::protocol.max_buffer_bytes`.
    ///
    /// ```
    /// use kimojio_http1::OutgoingBody;
    /// let body = OutgoingBody::full(b"hello".to_vec());
    /// ```
    ...
    ///
    /// ```
    /// use kimojio_http1::{Error, OutgoingBody, OutgoingFrame};
    /// let frames = futures::stream::iter([
    ///     Ok::<_, Error>(OutgoingFrame::Data(b"hello".to_vec())),
    ///     Ok(OutgoingFrame::Trailers(http::HeaderMap::new())),
    /// ]);
    /// let body = OutgoingBody::from_stream(None, frames);
    /// ```
...
/// Next item yielded by [`IncomingBody::frame`].
#[derive(Debug)]
pub enum IncomingFrame {
    /// Exclusive payload lease. Drop it to return the entire chunk to the driver.
    Data(BodyChunk),
    /// Validated trailer fields terminating the chunked body.
    Trailers(HeaderMap),
}
...
impl IncomingBody {
    ...
    /// Waits for the next payload lease or trailers; returns `None` at end of body.
    ///
    /// Call `accept` first if input delivery must begin before waiting for data.
    /// Drop each `Data` lease before expecting further reads on this connection.
    ...
    ///
    /// ```
    /// use kimojio_http1::{Error, IncomingBody};
    /// async fn collect_bounded(body: &mut IncomingBody) -> Result<Vec<u8>, Error> {
    ///     body.collect(1024 * 1024).await
    /// }
    /// ```
