...
impl<B: Buffer> Server<B> {
    /// Creates a server machine with one buffer type for input and output.
    ///
    /// `receive_buffer` must be initialized writable storage. `now` establishes
    /// the monotonic timestamp domain used for deadlines on this connection.
    pub fn new(
        id: ConnectionId,
        config: Config,
        receive_buffer: B,
...
impl<B: Buffer, W: AsRef<[u8]>> Server<B, W> {
    ...
    /// borrowed slice, or caller-selected shared storage. `receive_buffer`
    /// must be initialized writable storage with capacity for incoming bytes.
    ...
    /// Returns the identity assigned to this connection.
    ...
    /// Offers payload requested by `send_ready` and returns its receipt ID.
    ///
    /// If rejected, the original buffer and range are returned unchanged.
    ...
    /// Ends a producer body; supply trailers only for a streaming body.
    pub fn finish_body(
        &mut self,
        exchange: ExchangeId,
        trailers: Headers<'_>,
    ...
    /// Grants the application permission to receive up to `bytes` body bytes.
    ///
    /// Body delivery is backpressured by both granted credit and outstanding leases.
    ...
    /// Returns a delivered body lease with the number of bytes consumed.
    ///
    /// Only consumed bytes replenish credit; the unconsumed suffix may be offered again.
    pub fn release_body(
        &mut self,
        completion: BodyCompletion<B>,
    ) -> Result<(), Rejected<BodyCompletion<B>>> {
    ...
    /// Settles one outstanding read completion; rejected input is returned intact.
    ...
    /// Settles one outstanding write with its accepted byte count or transport error.
    #[allow(clippy::result_large_err)]
    ...
    /// Settles a readiness wait so the blocked operation can be issued again.
    ...
    /// Reports that the issued transport-close operation has settled.
    ...
    /// Reports a handler or body-source failure, including pending producer demand.
    pub fn fail_source(
        &mut self,
        exchange: ExchangeId,
        failure: Failure,
    ...
    /// Cancels application work for this exchange and closes it safely.
    pub fn cancel_exchange(&mut self, exchange: ExchangeId) -> Result<(), CommandError> {
        self.fail_source(exchange, Failure::Cancelled)
    }
    /// Stops admitting work according to `mode`; issued operations still must settle.
    pub fn shutdown(&mut self, mode: ShutdownMode) {
        self.core.shutdown(mode);
    }
    /// Records time without expiring deadlines; call before completion to let progress win.
    ...
    /// Applies the current deadline if it is due; stale or early tokens are rejected.
    pub fn expire(&mut self, deadline: Deadline, now: Tick) -> Result<(), CommandError> {
        self.core.expire(deadline, now)
    }
    /// Returns the connection identity and buffered input prefix after `upgrade_ready`.
...
impl<B: Buffer> Client<B> {
    /// Creates a client machine with one buffer type for input and output.
    ///
    /// `receive_buffer` must be initialized writable storage. `now` establishes
    /// the monotonic timestamp domain used for deadlines on this connection.
    pub fn new(
...
impl<B: Buffer, W: AsRef<[u8]>> Client<B, W> {
    ...
    /// Cancels this client exchange and waits for transport ownership to settle.
    pub fn cancel_exchange(&mut self, exchange: ExchangeId) -> Result<(), CommandError> {
        self.fail_source(exchange, Failure::Cancelled)
    }
    /// Stops new client requests and drains or aborts the active exchange.
    pub fn shutdown(&mut self, mode: ShutdownMode) {
        self.core.shutdown(mode);
    }
    /// Records time without expiring deadlines; use before completion to let progress win.
    pub fn observe_time(&mut self, now: Tick) -> Result<(), CommandError> {
        self.core.observe_time(now)
    }
    /// Advances time and applies all currently due phases without invoking ports.
    ...
    /// Applies the active client deadline if due; stale and early tokens are rejected.
    pub fn expire(&mut self, deadline: Deadline, now: Tick) -> Result<(), CommandError> {
        self.core.expire(deadline, now)
    }
    /// Returns the identity and buffered input prefix after a successful client upgrade.
    pub fn take_upgrade(&mut self) -> Result<Handoff<B>, CommandError> {
        self.core.take_upgrade()
    }
}
...
impl<B: Buffer, W: AsRef<[u8]>> Server<B, W> {
    ...
    /// Drives until a port callback yields, the machine closes, or no work remains.
    ///
    /// Uses manual deadline expiry; call `expire` explicitly or prefer `next_at`
    /// when deadlines should be enforced during each synchronous drive turn.
    ...
    /// Queues an informational response, such as `100 Continue` or `103 Early Hints`.
    ///
    /// Informational heads do not finish the exchange; send a final response separately.
    pub fn inform(
        &mut self,
        exchange: ExchangeId,
    ...
    /// Accepts an upgrade handshake and transfers the connection after its write settles.
    ///
    /// Call `take_upgrade` after the `upgrade_ready` callback to obtain buffered
    /// bytes that belong to the upgraded protocol.
...
impl<B: Buffer, W: AsRef<[u8]>> Client<B, W> {
    ...
    /// Admits one outbound request and returns its exchange identity.
    ///
    /// The request head is validated synchronously; body data is supplied later
    /// in response to `send_ready` unless attached eagerly before driving.
