...
mod test {
    ...
    #[test]
    fn manual_channel_test() {
        crate::run_test("manual_channel_test", async move {
    ...
    #[test]
    fn try_read_test() {
        crate::run_test("try_read_test", async move {
            let mut file = tempfile::tempfile().unwrap();
            file.write_all(b"hello world")
                .expect("Failed to write to file");
            file.seek(std::io::SeekFrom::Start(0)).unwrap();
            let mut stream =
                OwnedFdStream::new(unsafe { OwnedFd::from_raw_fd(file.into_raw_fd()) });
    ...
    #[test]
    fn short_read_test() {
        crate::run_test("short_read_test", async move {
            let mut file = tempfile::tempfile().unwrap();
            file.write_all(b"hello world")
                .expect("Failed to write to file");
            file.seek(std::io::SeekFrom::Start(0)).unwrap();
    ...
    #[test]
    fn read_failed_test() {
        crate::run_test("read_failed_test", async move {
            let (client, server) = bipipe();
    ...
    #[test]
    fn owned_fd_stream_split_test() {
        crate::run_test("owned_fd_stream_split_test", async move {
            let (client, server) = bipipe();
    ...
    #[test]
    fn owned_fd_stream_read_stream_try_read_test() {
        crate::run_test("owned_fd_stream_read_stream_try_read_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
    ...
    #[test]
    fn owned_fd_stream_read_stream_read_test() {
        crate::run_test("owned_fd_stream_read_stream_read_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (mut read_stream, _write_stream) = stream.split().await.unwrap();
    ...
    #[test]
    fn owned_fd_stream_write_stream_write_test() {
        crate::run_test("owned_fd_stream_write_stream_write_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (_read_stream, mut write_stream) = stream.split().await.unwrap();
    ...
    #[test]
    fn owned_fd_stream_write_stream_writev_test() {
        crate::run_test("owned_fd_stream_write_stream_writev_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (_read_stream, mut write_stream) = stream.split().await.unwrap();
    ...
    #[test]
    fn owned_fd_stream_write_stream_shutdown_test() {
        crate::run_test("owned_fd_stream_write_stream_shutdown_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (_read_stream, mut write_stream) = stream.split().await.unwrap();
    ...
    #[test]
    fn owned_fd_stream_write_stream_close_test() {
        crate::run_test("owned_fd_stream_write_stream_close_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (_read_stream, mut write_stream) = stream.split().await.unwrap();
    ...
    #[test]
    fn owned_fd_stream_read_stream_close_test() {
        crate::run_test("owned_fd_stream_read_stream_close_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (mut read_stream, _write_stream) = stream.split().await.unwrap();
    ...
    #[test]
    fn owned_fd_stream_split_concurrent_operations_test() {
        crate::run_test(
            "owned_fd_stream_split_concurrent_operations_test",
            async move {
                let (client, server) = bipipe();
    ...
    #[test]
    fn owned_fd_stream_split_writev_large_test() {
        crate::run_test("owned_fd_stream_split_writev_large_test", async move {
            let (client, server) = bipipe();
            let stream = OwnedFdStream::new(client);
            let (_read_stream, mut write_stream) = stream.split().await.unwrap();
