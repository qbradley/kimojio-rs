...
#[test]
fn superseded_alarm_only_settles_before_or_after_cancellation_dispatch() {
    for acknowledge_first in [false, true] {
        for replacement in [None, Some(50), Some(200)] {
            for outcome in [
                WakeOutcome::Fired(Duration::ZERO),
                WakeOutcome::Fired(Duration::from_secs(1_000_000)),
                WakeOutcome::Failed(IoFailure::Failed),
                WakeOutcome::Failed(IoFailure::Cancelled),
            ] {
                for dispatch_first in [false, true] {
                    ...
                    let completion = match outcome {
                        WakeOutcome::Fired(now) => original.complete(now),
                        WakeOutcome::Failed(error) => original.failed(error),
                    };
                    pair.client.complete_wake(completion).unwrap();
                    ...
                    assert_eq!(
                        pair.client_ports.cancels.len(),
                        usize::from(dispatch_first && !acknowledge_first)
                    );
