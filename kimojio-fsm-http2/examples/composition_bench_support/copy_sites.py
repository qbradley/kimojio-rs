...
assert hashlib.sha256((frozen / "composition_bench").read_bytes()).hexdigest() == \
    "205f19afeaa93d7d6161a3f32dd3359098bf2805d2b0bca08c5cd8a7c077c527", \
    "the inline-move address belongs only to the d4bc869 frozen binary"
profiles = ["direct-empty", "selected-empty", "direct-empty128", "direct-1m"]
samples = {}
for name in profiles:
    by_symbol = collections.defaultdict(list)
    text = re.sub(r"\[[0-9a-f]{16}\]", "", (frozen / f"{name}.demangled.txt").read_text())
